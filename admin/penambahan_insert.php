<?php 
    ...
    $panggil_query = "select * from supplier";
        ...
        <div class="card shadow mb-4">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="kebutuhan" name="kebutuhan" placeholder="Input Kebutuhan Penambahan">
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="nm_supplier" id="nm_supplier">
                            <?php
                                if(mysqli_num_rows($query_supplier)>0) 
                                {
                                    while($isi = mysqli_fetch_assoc($query_supplier))
                                    {
                                    ?>
                                        <option value="<?php echo $isi['kode_sup'] ?>"> <?php echo $isi['nama_sup'] ?> </option>
                                    <?php
                                    }
                                }
                            ?>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" id="estimasi_datang" name="estimasi_datang" placeholder="Input Estimasi Datang">
                        </div>
                    </div>
                </div>
                <hr> 
			...
			<div class="mb-3 row" id="tableAddItem">
				...
				<div class="col-md-3 mt-3 mb-3">
					<label for="harga" class="form-label input-group">Harga Pasaran/Satuan (RP):</label>
					<input type="text" name="harga" id="harga" class="form-control input-group" readonly />
				</div>
				<div class="col-md-2 mt-3 mb-3">
					<label for="tmb_stok" class="form-label input-group">Tambah Stok:</label>
			...
			<div class="">
				...
				<button id="btnsave_penambahan" class="btn btn-primary">Lanjutkan Pengajuan</button>
			</div>
            </div>
        </div>
        </div>
	...
	$(document).ready(function() {
		...
		$.fn.saveBarang = function() 
		{
			...
			if (sameItem === false) {
				...
				datatablemyTable.row.add([kode, nama, stok, satuan, harga, tmb_stok, totalhrg, `<button id="id${panjang}" class="remove btn btn-danger">X</button>`]).draw();
				...
				kolomData[indexSameRow][5] = tmb_stok;
        ...
        {
			$('#btnsave_penambahan').on('click', function() 
            {
				//deklarasikan variabel data penambahan
				let kode_penambahan = $.fn.ambilWaktu();
				let kebutuhan = $('#kebutuhan').val();
				let nm_supplier = $('#nm_supplier').val();
				let estimasi_datang = $('#estimasi_datang').val();
				let total = $('#totalHarga').text();

				let data = {
					kode_penambahan: kode_penambahan,
					kebutuhan: kebutuhan,
					nm_supplier: nm_supplier,
					estimasi_datang: estimasi_datang,
					total: total
				};

				axios.post('penambahan_proses/add_penambahan.php', JSON.stringify(data), {
					headers: {
				...
				$('#myTable tbody tr').each(function() {
					...
					let harga = currentRow.find('td').eq(4).text();
					let tmb_stok = currentRow.find('td').eq(5).text();
					let totalhrg = currentRow.find('td').eq(6).text();

					axios.post('penambahan_proses/add_detailpenambahan.php', JSON.stringify({
				...
				window.location.href = 'penambahan.php';
			})
		}
		// ==================================================
